...
export function MugogoLogo({ size = 36, color = '#8c6e3f', withText = false, textColor = '#2e1f10' }: LogoProps) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
      ...
      {withText && (
        <div>
          <div style={{ fontFamily:'Georgia,serif', fontSize:`${size*0.38}px`, fontWeight:700, color:textColor, lineHeight:1.2 }}>
            Concession Mugogo
          </div>
          <div style={{ fontSize:`${size*0.24}px`, color:'#8c6e3f', marginTop:'1px' }}>
            Walungu, Sud-Kivu
          </div>
        </div>
      )}
...
export function getReportLogoHTML(color = '#8c6e3f'): string {
  return `<div style="display:flex;align-items:center;gap:14px">
    ...
    <div>
      <div style="font-family:Georgia,serif;font-size:22px;font-weight:700;color:${color}">Concession Mugogo</div>
      <div style="font-size:11px;color:#666">Gestion Agro-pastorale · Walungu, Sud-Kivu, RDC</div>
      <div style="font-size:11px;color:#888">+243 976960983 | richardbunani2013@gmail.com</div>
    </div>
  </div>`
}
